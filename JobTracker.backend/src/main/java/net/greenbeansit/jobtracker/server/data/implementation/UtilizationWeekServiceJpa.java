...
import net.greenbeansit.jobtracker.server.data.UtilizationWeekDataService;
import net.greenbeansit.jobtracker.server.data.entity.UtilizationWeekEntity;
import net.greenbeansit.jobtracker.server.data.repository.UtilizationWeekEntityRepository;
...
public class UtilizationWeekServiceJpa implements UtilizationWeekDataService
{
	...
	{
		ArrayList<UtilizationWeek> list = new ArrayList<UtilizationWeek>();
		for (UtilizationWeekEntity entity : repository.findByPseudoJobId(pseudoJobId))
		{
			list.add(convert(entity));
		}
		return list;
